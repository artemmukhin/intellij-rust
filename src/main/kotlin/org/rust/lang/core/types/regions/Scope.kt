...
fun getRegionScopeTree(item: RsItemElement): ScopeTree {
    val visitor = RegionResolutionVisitor(item)
    item.accept(visitor)
...
sealed class Scope {
    ...
    data class CallSite(override val element: RsBlock) : Scope()

    /** Scope of arguments passed to a function or closure (they outlive its body). */
    data class Arguments(override val element: RsBlock) : Scope()

    /**
     * Represents the scope of user code running immediately after the initializer expression for the indexed stmt,
     * until the end of the block.
     */
    data class Remainder(override val element: RsBlock, val firstStatementIndex: Int) : Scope()
}
...
val Scope.span: TextRange
    get() {
        ...
        if (this is Scope.Remainder) {
            // Want span for scope starting after the indexed stmt and ending at end of `block`;
            // reuse span of `block` and shift `startOffset` forward to end of indexed stmt.
            val stmtSpan = element.itemOrStmtChildren[firstStatementIndex].textRange

            // To avoid issues with macro-generated spans, the span of the stmt must be nested.
            if (span.startOffset <= stmtSpan.startOffset && stmtSpan.endOffset <= span.endOffset) {
                return TextRange(stmtSpan.endOffset, span.endOffset)
            }
        }
        return span
    }
...
data class ScopeTree(
    ...
    private val rootBody: RsBlock? = null,
    ...
    fun getFreeScope(region: ReFree): Scope {
        val parameterOwner = if (region.bound is BoundRegion.Named) {
            region.bound.item.ancestorStrict<RsItemElement>()
        } else {
            region.item
        }
        check(parameterOwner === region.item)
...
class RegionResolutionVisitor(item: RsItemElement) : RsVisitor() {
    var ctx: Context = Context()

    /** Generated scope tree. */
    val scopeTree: ScopeTree = run {
        val rootBody = item.body
        val rootParent = item.ancestorStrict<RsItemElement>() as? RsTraitOrImpl
    ...
    override fun visitFunction(fn: RsFunction) {
        ...
        enterScope(Scope.Arguments(body))
    ...
    override fun visitBlock(block: RsBlock) {
        ...
        for ((index, element) in block.itemOrStmtChildren.withIndex()) {
            if (element is RsItemElement || element is RsLetDecl) {
                // Each item or stmt introduces a subscope for bindings introduced by the declaration.
                // Each subscope in a block has the previous subscope in the block as a parent, except for the first such
                // subscope, which has the block itself as a parent.
                enterScope(Scope.Remainder(block, index))
                ctx.variableParent = ctx.parent
            }
            element.accept(this)
        }
