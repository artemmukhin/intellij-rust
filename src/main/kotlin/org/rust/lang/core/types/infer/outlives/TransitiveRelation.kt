...
class TransitiveRelation<T> {
    ...
    private fun getMinimalUpperBounds(element1: T, element2: T): MutableList<T> {
        ...
        return leastUpperBoundIndices.reversed().map { elements[it] }.toMutableList()
    }
...
class BitMatrix(private val rows: Int, private val columns: Int) {
    ...
    fun intersection(row1: Int, row2: Int): MutableList<Int> {
        ...
        val bits1 = bits.get(getIndex(row1), getIndex(row1 + 1))
        val bits2 = bits.get(getIndex(row2), getIndex(row2 + 1))
    ...
    fun merge(source: Int, target: Int): Boolean {
        ...
        val sourceBits = bits.get(getIndex(source), getIndex(source + 1))
        val targetBits = bits.get(getIndex(target), getIndex(target + 1))
...
private fun BitSet.toMutableList(): MutableList<Int> = stream().mapToObj { it }.collect(Collectors.toList())
